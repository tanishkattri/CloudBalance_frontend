...
const FilterSideBar = ({ sidebarLoading, setSidebarLoading, filterOptions, setFiltersMap, filtersMap }) => {
  ...
  const fetchInnerFilters = async (filterName) => {
    ...
    try {
      ...
      const res = await getApi(`snowflake/filters?groupBy=${encodeURIComponent(trimmedFilter)}`);
  ...
  const handleClose = () => {
    setExpandedFilter(null); // Don't save anything, discard local changes
  };

  return (
    <aside className="relative w-[300px] bg-white border-l shadow-md p-4 overflow-y-auto">
      {sidebarLoading && <ScopeLoader />}
      <nav className={`${sidebarLoading ? "opacity-30 pointer-events-none" : ""}`}>
        <ul>
          {filterOptions.map((filter) => {
            ...
            return (
              <li key={filter.id} className="p-2 rounded hover:bg-gray-100">
                ...
                >
                  <span className="text-sm font-medium">{filter.displayName}</span>
                  <span className="text-xs">{expandedFilter === filter.groupName ? "▲" : "▼"}</span>
                </div>

                {/* Inner Filter Checkboxes */}
                {expandedFilter === filter.groupName && innerFilters[filter.groupName] && (
                  <>
                    <ul className="ml-4 mt-2 space-y-1 max-h-40 overflow-y-auto">
                      {innerFilters[filter.groupName].map((val, idx) => (
                        <li key={idx} className="flex items-center">
                          <input
                            ...
                            className="mr-2"
                            checked={
                              localSelected[filter.groupName]?.includes(val) ||
                              false
                            }
                            onChange={() =>
                              handleInnerCheckboxChange(filter.groupName, val)
                            }
                          />
